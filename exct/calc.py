...
def weightCalc(vMap:dict[str,int]) -> float|None:
	try:
		C25:float = getValueIfInMap(vMap, "Body Height");
		C31:float = getValueIfInMap(vMap, "Lower Torso Width")
		E32:float = getValueIfInMap(vMap, "Belly Thickness")
		D29:float = getValueIfInMap(vMap, "Shoulder Thickness")
		D30:float = getValueIfInMap(vMap, "Upper Torso Thickness")
		C32:float = getValueIfInMap(vMap, "Waist Width")
		C33:float = getValueIfInMap(vMap, "Hip Width")
		C35:float = getValueIfInMap(vMap, "Upper Thigh Width")
		D35:float = getValueIfInMap(vMap, "Upper Thigh Thickness")
		D33:float = getValueIfInMap(vMap, "Hip Thickness")
		C34:float = getValueIfInMap(vMap, "Butt Size")
		C25:float = getValueIfInMap(vMap, "Body Height");
		C29:float = getValueIfInMap(vMap, "Shoulder Width");
		C30:float = getValueIfInMap(vMap, "Upper Torso Width");
		D29:float = getValueIfInMap(vMap, "Shoulder Thickness");
		D30:float = getValueIfInMap(vMap, "Upper Torso Thickness");
		E27:float = getValueIfInMap(vMap, "Breast Roundness");
		C27:float = getValueIfInMap(vMap, "Breast Size");
		D27:float = getValueIfInMap(vMap, "Breast Depth");
		D31:float = getValueIfInMap(vMap, "Lower Torso Thickness");
		D32:float = getValueIfInMap(vMap, "Waist Thickness");
	...
	O7 = 0.9
	Q7 = L7/(1+O7/3)
	L14 = 2869/3000 + 131/150000 * C25
	L20 = (C29*0.9+C30*1.1)/2
	if 0 <= L20 <= 100:
		P20 = L20/3 + L20 / 150
	else:
		P20 = L20
	...
	P9 = 0.46
	R9 = N9/(1+P9/3)
	M31 = R9*(1+Q22/100*P9)*L14*(0.95+C34/1000)*(0.975+K24/2000)
	...
	G27 = 100
	O52 = 14/15+C25*1/750
	M52 = C27/50*N52*L49*G27/100*O52
	K49 = D27/10000*30+0.85
	if C27 > 50:
		L52 = (N52*2/3*(1+(C27-50)/25))*K49*G27/100*O52
	else:
		L52 = (N52*2/3*C27/50)*K49*G27/100*O52
	K52 = (((M52/2)**2*(L52/3))*2/3*maths.pi) + ((M52/2)**2 * maths.pi * L52 * (2 / 3) / 3)
	E44 = K52*0.918*2
	F48 = 0.53
	F49 = 0.15
